...
$('choose-folder').onclick = async () => { try { const result = await api.chooseFolder(mode); if (result) { chosen = result; $('folder-value').textContent = result.folder + (result.fileStem ? ` / ${result.fileStem}.fwl` : ''); if (!$('name').value && result.fileStem) $('name').value = result.fileStem; } } catch (e) { toast(e.message); } };
