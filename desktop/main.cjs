...
function notify() { if (win && !win.isDestroyed()) win.webContents.send('state', client.state()); }
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    const poll = setInterval(() => client.tick().catch(console.error), 15_000);
    ...
    win.on('closed', () => { clearInterval(poll); clearInterval(heartbeat); app.quit(); });
